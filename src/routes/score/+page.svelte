	...
	function fu(a) {
		...
		if (result == 20 && intersection(a1, a[3]).length > 0) {
			if (a[4][0] == 2) {
				...
				PIN = FIN;
				if (FIN < WIN) WIN = FIN;
				ZWIN = 'Success! Your time is ';
				WINNERS = WIN;
				// top.set([WIN]);
				setTimeout(() => runRoller(),1000);
...
<div style="font-family: Times New Roman; text-align:center; font-size: 32px;" transition:fade>
	...
	The State of the game of Score in an RC_Monad
</div>
<br />
...
<!-- <h1>{$userName}</h1> -->
<br />
<span>Best Score: {WIN} </span>
<h2>{ZWIN} {d3} seconds</h2>
...
<span style="margin-left: 64px; font-size: 32px">Round: {m2(s)[4]*1 + 1}</span>
</li>
</ul>
<br />
<ul>
<li>
<button on:click={() => m2(clic0)}>+</button>
</li>
<li>
<button on:click={() => m2(clic1)}>-</button>
</li>
<li>
<button on:click={() => m2(clic2)}>*</button>
</li>
<li>
<button on:click={() => m2(clic3)}>/</button>
</li>
<li>
<button on:click={() => m2(clic4)}>concat</button>
</li>
...
<p>
	Just computing 20 won't complete a round. One of the numbers used to get 20 has to be the result of a prior computation. For example, (4+6)*2 or (7-6)*20.
</p>

<p> The game is held in the recursive closure returned initially by M(x) where M is: </p>
<pre>{monad}</pre>
<p>and x is the array of eight arrays shown below. The function m2 forms a closure with M(x) and is defined as:</p>
<pre>{start}</pre>

	<h2>Background</h2>
	<span style="text-indent: 3%">Score is a game involving four dice and two or three-stage arithmetic computations with the goal of arriving at the number 20. My son Alex taught it to me a decade ago, when he was in middle school. I decided to make an online multi-player version. </span>
